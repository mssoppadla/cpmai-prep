...
| 38 | SOME users report "connection timed out" loading the site while curl/monitoring shows it healthy everywhere (200s in ~200 ms, no container restarts, no OOM, no 5xx) | Host Caddy listens on **udp/443 (QUIC)** and stamps `Alt-Svc: h3=":443"; ma=2592000` on every response, but ufw only allowed 443/**tcp** — inbound UDP was silently dropped. Browsers cache the h3 hint for 30 days; on later visits they try QUIC first, the packets vanish, and clients that don't race the TCP fallback cleanly hang until "connection timed out". Intermittent + per-user by nature, so the site always looks fine from probes | `provision.sh` now opens `443/udp` alongside `443/tcp` (2026-08-03). Rule applied live the same day; verified end-to-end by watching the ufw ACCEPT counter tick while sending UDP datagrams from outside | Rule of thumb: **never advertise what the firewall eats** — if Caddy serves h3, UDP 443 must be open (or disable h3 explicitly). To confirm the symptom class from a report: ask whether a hard-refresh in a private window works (fresh profile has no cached h3 hint → always TCP → works) |
| 39 | Applying the repo `infra/Caddyfile` to the live Caddy 502'd ALL of prod (2026-08-07). Also: QUIC errors persisted for some users even after row 38's ufw fix | Two drifts at once. (a) The live `/etc/caddy/Caddyfile` had never matched the repo file: prod publishes containers on host ports **3001/8001** (`FRONTEND_HOST_PORT`/`BACKEND_HOST_PORT` in the VPS env, per row 1), repo said 3000/8000. (b) Repo upstreams said `localhost`, which Caddy dials as `::1` first while compose publishes IPv4 `127.0.0.1` only → connection refused. Separately, ufw-allowing udp/443 didn't stop QUIC stalls — UDP dies elsewhere on some paths (provider edge, client networks) | h3 disabled globally in the Caddyfile (`servers { protocols h1 h2 }`) — never advertised means never stalled, regardless of where UDP dies. Repo Caddyfile pins `127.0.0.1:3001` / `127.0.0.1:8001` and matches the persisted `/etc/caddy/Caddyfile` byte-for-byte. `deploy.sh` prints a loud warning at the end of every deploy when the two files diverge | **Before ANY Caddy change:** (1) `diff infra/Caddyfile /etc/caddy/Caddyfile` first — the live file is the truth about ports/hosts; (2) apply via the no-sudo admin API (`caddy adapt --config infra/Caddyfile \| curl -X POST localhost:2019/load ...`) — it rejects bad configs and keeps the old one running; (3) verify with a STATUS CODE (`curl -s -o /dev/null -w "%{http_code}" https://cpmaiexamprep.com`), never by grepping headers — a 502 "passes" a header grep; (4) persist with `sudo cp` + `caddy validate` + `systemctl reload` (reload, never restart) or a reboot silently reverts the hot-loaded config |
| 40 | Rollback restore silently WIPED all prod uploads (testimonial photos, lesson videos 404'd), yet printed "✓ uploads restored". Separately: the rollback claimed "backend did not come back up" while it was healthy, and disk jumped 67%→81% in a day | Four independent script defects. (a) restore.sh cleared `/app/uploads` FIRST, then ran `gunzip -c \| tar -xzf` — a double decompression that always fails (`stdin: not in gzip format`) — then printed success unconditionally. (b) The next pre-deploy backup archived the now-empty dir into a valid 106-byte tarball with no warning, certifying the loss. (c) restore.sh health-checked hardcoded port 8000 while prod publishes 8001 (`.deploy.conf`). (d) Retention kept 30 daily 2 GB uploads tarballs (~60 GB) and never pruned manual-tag env/uploads archives | restore.sh now stages the archive into `/app/uploads.restore-tmp`, verifies extraction + non-zero file count, and only then swaps — a bad archive is a no-op, never a wipe; refuses <1KB archives; fails loudly. backup.sh shouts when the uploads snapshot is <1KB. Health check reads BACKEND_HOST_PORT from `.deploy.conf`. Retention: 7 daily uploads tarballs, 5 pre-deploy uploads tarballs, manual tags pruned across all three suffixes | Recovery, if uploads ever vanish again: pick the newest FULL-SIZE `.uploads.tar.gz` (`ls -lt /var/backups/cpmai-prep/*.uploads.tar.gz` — ignore ~100-byte ones) and `docker compose … exec -T backend tar -xzf - -C /app/uploads < <file>`. General rule: **never trust a "✓" a script prints after `\|\| warn` — and never wipe before verifying the replacement extracts** |
