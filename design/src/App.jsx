...
export default function App() {
  ...
  const adminNav = [
    ...
    { id: 'admin-exam-sets', label: 'Exam Sets', icon: GraduationCap, badge: 3 },
    { id: 'admin-questions', label: 'Questions', icon: BookOpen, badge: 5 },
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <main className="flex-1 overflow-auto min-w-0">
        ...
        {view === 'admin-exam-sets' && <ExamSetsAdmin />}
        {view === 'admin-questions' && <QuestionsAdmin />}
...
// -----------------------------------------------------------------------------
// Exam Sets — list + drill-down to manage linked questions (reorder, add, remove)
// -----------------------------------------------------------------------------
function ExamSetsAdmin() {
  const [selectedSet, setSelectedSet] = useState(null);
  const [editingSet, setEditingSet] = useState(null);
  const [creating, setCreating] = useState(false);

  const sets = [
    { id: 1, name: 'Set 1 — Foundations', slug: 'set-1-foundations',
      description: 'First-attempt-friendly mix covering all 6 CPMAI phases at medium difficulty.',
      difficulty: 'medium', timeLimit: 60, passing: 70, premium: false, active: true,
      displayOrder: 10, linked: 5, attempts: 412 },
    { id: 2, name: 'Set 2 — Advanced', slug: 'set-2-advanced',
      description: 'Hardest 60 questions across all phases. Premium plan required.',
      difficulty: 'hard', timeLimit: 90, passing: 70, premium: true, active: true,
      displayOrder: 20, linked: 5, attempts: 138 },
    { id: 3, name: 'Set 3 — Phase Drill: Modeling', slug: 'set-3-phase-modeling',
      description: 'Deep dive on CPMAI Phase 4 (Modeling) only. Targeted practice.',
      difficulty: 'medium', timeLimit: 30, passing: 75, premium: false, active: true,
      displayOrder: 30, linked: 5, attempts: 241 },
  ];

  if (selectedSet) {
    return <ExamSetDetail set={selectedSet} onBack={() => setSelectedSet(null)} />;
  }
  ...
  return (
    <div className="p-8">
      <PageHeader
        title="Exam Sets"
        subtitle="Curate exam sets that candidates take. Link questions, reorder, and toggle availability."
        action={
          <button onClick={() => setCreating(true)}
            className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700">
            <Plus className="w-4 h-4" />New Exam Set
          </button>
        }
      />
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <table className="w-full">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider">
              <th className="px-5 py-3">Exam Set</th>
              <th className="px-5 py-3">Difficulty</th>
              <th className="px-5 py-3 text-right">Questions</th>
              <th className="px-5 py-3 text-right">Time / Pass</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {sets.map(s => (
              <tr key={s.id} className="hover:bg-slate-50">
                <td className="px-5 py-4">
                  <button onClick={() => setSelectedSet(s)}
                    className="text-left">
                    <div className="text-sm font-medium text-slate-900 hover:text-indigo-600">{s.name}</div>
                    <div className="text-xs text-slate-500 mt-0.5 flex items-center gap-1.5">
                      <code className="bg-slate-100 px-1.5 py-0.5 rounded">{s.slug}</code>
                    </div>
                    <div className="text-xs text-slate-500 mt-1 max-w-md truncate">{s.description}</div>
                  </button>
                </td>
                <td className="px-5 py-4">
                ...
                <td className="px-5 py-4 text-right">
                  <button onClick={() => setSelectedSet(s)}
                    className="text-sm font-medium text-indigo-600 hover:underline tabular-nums">
                    {s.linked} linked →
                  </button>
                </td>
                <td className="px-5 py-4 text-right text-sm text-slate-600 tabular-nums">
                  {s.timeLimit}m · {s.passing}%
                </td>
                <td className="px-5 py-4">
                  <div className="flex flex-col gap-1">
                    {s.active ? <Badge color="green" dot="green">active</Badge>
                              : <Badge color="slate">disabled</Badge>}
                    {s.premium && <Badge color="purple">premium</Badge>}
                  </div>
                </td>
                <td className="px-5 py-4 text-right text-sm text-slate-600 tabular-nums">{s.attempts.toLocaleString()}</td>
                <td className="px-5 py-4 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button title="Manage Questions" onClick={() => setSelectedSet(s)}
                      className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded">
                      <BookOpen className="w-4 h-4" />
                    </button>
                    <button title="Edit" onClick={() => setEditingSet(s)}
      ...
      <div className="mt-4 text-xs text-slate-500 flex items-center gap-4">
        <span className="flex items-center gap-1.5"><GraduationCap className="w-3 h-3" /> Click a set name or "linked" count to manage its questions</span>
        <span>·</span>
        <span>Display order controls candidate-facing list ordering</span>
      </div>

      {(creating || editingSet) && (
        <ExamSetEditor onClose={() => { setCreating(false); setEditingSet(null); }} editing={editingSet} />
      )}
    </div>
  );
}

function ExamSetDetail({ set, onBack }) {
  const phaseColors = { BU: 'green', DU: 'blue', DP: 'purple', MD: 'indigo', EV: 'yellow', DE: 'red' };
  const linked = [
    { id: 11, position: 1, phase: 'DU', difficulty: 'medium', stem: 'In CPMAI Phase 2 (Data Understanding), you discover 18% of records have missing values in a key feature. What is the MOST APPROPRIATE immediate action?' },
    { id: 12, position: 2, phase: 'BU', difficulty: 'easy',   stem: 'Which CPMAI phase explicitly produces the formal business goal and success criteria for the AI initiative?' },
    { id: 13, position: 3, phase: 'EV', difficulty: 'medium', stem: 'Which evaluation metric is generally MOST appropriate for a binary classifier on a heavily imbalanced dataset where false negatives are very costly?' },
    { id: 14, position: 4, phase: 'DE', difficulty: 'medium', stem: 'A model performs well in offline evaluation but degrades in production. According to CPMAI Phase 6, what is the FIRST thing to investigate?' },
    { id: 15, position: 5, phase: 'DP', difficulty: 'hard',   stem: 'When preparing data in CPMAI Phase 3, you encounter a categorical feature with 1,200 distinct values. Which technique is MOST APPROPRIATE before passing it to a tree-based model?' },
  ];
  const unlinked = [
    { id: 21, phase: 'MD', difficulty: 'medium', stem: 'During CPMAI Phase 4, when should you choose ensemble methods over a single model?' },
    { id: 22, phase: 'BU', difficulty: 'hard',   stem: 'How should AI project ROI be measured when benefits are largely qualitative?' },
  ];
    ...
    <div className="p-8 max-w-5xl">
      ...
      <PageHeader
        ...
        subtitle={
          <span className="flex items-center gap-2 flex-wrap">
            ...
            <span>·</span>
            <span>{linked.length} questions</span>
            <span>·</span>
            <span>{set.timeLimit} min · pass {set.passing}%</span>
            {set.premium && <Badge color="purple">premium</Badge>}
            <Badge color="green" dot="green">active</Badge>
          </span>
        }
        action={
          <div className="flex items-center gap-2">
            <button className="px-3 py-2 bg-white text-slate-700 text-sm font-medium border border-slate-300 rounded-lg hover:bg-slate-50">
              Edit set details
            </button>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
        <div className="mb-4">
          ...
          <p className="text-sm text-slate-600">This is the order learners will see. Use ↑/↓ to reorder, then save.</p>
        </div>
            ...
            <li key={q.id} className="flex items-start gap-3 py-3">
              <div className="flex flex-col items-center w-8 select-none">
                <button title="Move up" disabled={i === 0}
                  className="text-slate-400 hover:text-indigo-600 disabled:opacity-30 text-sm leading-none">▲</button>
                <span className="text-xs text-slate-500 tabular-nums my-1">{i + 1}</span>
                <button title="Move down" disabled={i === linked.length - 1}
                  className="text-slate-400 hover:text-indigo-600 disabled:opacity-30 text-sm leading-none">▼</button>
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm text-slate-900 line-clamp-2">{q.stem}</div>
                <div className="text-xs text-slate-500 mt-1 flex items-center gap-2">
                  <Badge color={phaseColors[q.phase]}>{q.phase}</Badge>
                  <span>·</span>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <div className="flex items-center justify-between mb-1">
          <h2 className="font-semibold text-slate-900">Add questions</h2>
        ...
        <p className="text-sm text-slate-600 mb-3">Pick from the question bank. Already-linked questions are filtered out.</p>
        {showPicker && (
          <>
            ...
            <div className="border border-slate-200 rounded-lg max-h-72 overflow-y-auto">
              <ul className="divide-y divide-slate-100">
                {unlinked.filter(q => !search || q.stem.toLowerCase().includes(search.toLowerCase())).map(q => {
                  const sel = picked.has(q.id);
                  return (
                    <li key={q.id} onClick={() => togglePick(q.id)}
                      className="flex items-start gap-3 p-3 hover:bg-slate-50 cursor-pointer">
                      <input type="checkbox" checked={sel} readOnly className="mt-1 pointer-events-none" />
                      <div className="flex-1 min-w-0">
                        <div className="text-sm text-slate-900 line-clamp-2">{q.stem}</div>
                        <div className="text-xs text-slate-500 mt-1 flex items-center gap-2">
                          <Badge color={phaseColors[q.phase]}>{q.phase}</Badge>
            ...
            <div className="flex items-center justify-between mt-4">
              <div className="text-sm text-slate-600">{picked.size} selected</div>
...
function ExamSetEditor({ onClose, editing }) {
  return (
    <div onClick={onClose}
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <h2 className="text-lg font-bold text-slate-900 mb-4">
          {editing ? `Edit "${editing.name}"` : 'New exam set'}
        </h2>
        <div className="space-y-3">
          <Field label="Name">
            <input defaultValue={editing?.name ?? ''} placeholder="Set 4 — Phase Drill: Evaluation"
          ...
          <Field label="Slug (URL identifier)">
            <input defaultValue={editing?.slug ?? ''} placeholder="set-4-phase-evaluation"
              className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" />
          </Field>
          <Field label="Description">
            <textarea rows={2} defaultValue={editing?.description ?? ''}
              className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Difficulty">
              <select defaultValue={editing?.difficulty ?? 'medium'}
                className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg">
                <option value="easy">easy</option><option value="medium">medium</option><option value="hard">hard</option>
              </select>
            </Field>
            <Field label="Time limit (min)">
              <input type="number" defaultValue={editing?.timeLimit ?? 90}
                className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg" />
            </Field>
            <Field label="Passing score (%)">
              <input type="number" defaultValue={editing?.passing ?? 70}
                className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg" />
            </Field>
            <Field label="Display order">
              <input type="number" defaultValue={editing?.displayOrder ?? 100}
                className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg" />
            </Field>
          </div>
          <div className="flex items-center gap-4 pt-1">
            <label className="flex items-center gap-2 text-sm text-slate-700">
              <input type="checkbox" defaultChecked={editing?.active ?? true} />Active
            </label>
            <label className="flex items-center gap-2 text-sm text-slate-700">
              <input type="checkbox" defaultChecked={editing?.premium ?? false} />Premium
            </label>
          </div>
        </div>
        <div className="flex justify-end gap-2 mt-6">
          <button onClick={onClose}
            className="px-4 py-2 text-sm text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50">Cancel</button>
          <button onClick={onClose}
...
// -----------------------------------------------------------------------------
// Questions — bank list with filters + per-question editor preview
// -----------------------------------------------------------------------------
function QuestionsAdmin() {
  ...
  const phases = [
    { code: 'BU', name: 'Business Understanding' },
    { code: 'DU', name: 'Data Understanding' },
    { code: 'DP', name: 'Data Preparation' },
    { code: 'MD', name: 'Modeling' },
    { code: 'EV', name: 'Model Evaluation' },
    { code: 'DE', name: 'Model Operationalization' },
  ];
  const phaseColors = { BU: 'green', DU: 'blue', DP: 'purple', MD: 'indigo', EV: 'yellow', DE: 'red' };

  const questions = [
    { id: 11, phase: 'DU', difficulty: 'medium', active: true, sets: 3, options: 4,
      stem: 'In CPMAI Phase 2 (Data Understanding), you discover 18% of records have missing values in a key feature. What is the MOST APPROPRIATE immediate action?',
      domain: 'Data Understanding > Data Quality Assessment' },
    { id: 12, phase: 'BU', difficulty: 'easy', active: true, sets: 3, options: 4,
      stem: 'Which CPMAI phase explicitly produces the formal business goal and success criteria for the AI initiative?',
      domain: 'Business Understanding > Goal Definition' },
    { id: 13, phase: 'EV', difficulty: 'medium', active: true, sets: 3, options: 4,
      stem: 'Which evaluation metric is generally MOST appropriate for a binary classifier on a heavily imbalanced dataset where false negatives are very costly?',
      domain: 'Model Evaluation > Metric Selection' },
    { id: 14, phase: 'DE', difficulty: 'medium', active: true, sets: 3, options: 4,
      stem: 'A model performs well in offline evaluation but degrades in production. According to CPMAI Phase 6, what is the FIRST thing to investigate?',
      domain: 'Model Operationalization > Production Monitoring' },
    { id: 15, phase: 'DP', difficulty: 'hard', active: true, sets: 3, options: 4,
      stem: 'When preparing data in CPMAI Phase 3, you encounter a categorical feature with 1,200 distinct values. Which technique is MOST APPROPRIATE before passing it to a tree-based model?',
      domain: 'Data Preparation > Feature Engineering' },
    { id: 21, phase: 'MD', difficulty: 'medium', active: false, sets: 0, options: 4,
      stem: 'During CPMAI Phase 4, when should you choose ensemble methods over a single model?',
      domain: 'Modeling > Technique Selection' },
  ];

  const filtered = questions.filter(q =>
    (!filterPhase || q.phase === filterPhase) &&
  ...
  return (
    <div className="p-8">
      <PageHeader
        title="Question Bank"
        subtitle="Author MCQs once. Link them to any number of exam sets."
        action={
          <button onClick={() => setCreating(true)}
            className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-3 mb-4 flex gap-2">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Search question stem…"
            className="w-full pl-9 pr-3 py-1.5 text-sm border border-slate-300 rounded" />
        </div>
        ...
        <table className="w-full">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider">
              ...
              <th className="px-5 py-3">Difficulty</th>
              <th className="px-5 py-3 text-right">In Sets</th>
              <th className="px-5 py-3">Status</th>
              <th className="px-5 py-3"></th>
            </tr>
          </thead>
            ...
            {filtered.map(q => (
              <tr key={q.id} className="hover:bg-slate-50">
                <td className="px-5 py-4 max-w-xl">
                  <div className="text-sm text-slate-900 line-clamp-2">{q.stem}</div>
                  <div className="text-xs text-slate-500 mt-0.5">{q.domain} · {q.options} options</div>
                </td>
                <td className="px-5 py-4">
                  <Badge color={phaseColors[q.phase]}>{q.phase}</Badge>
                </td>
                <td className="px-5 py-4 text-sm text-slate-600 capitalize">{q.difficulty}</td>
                <td className="px-5 py-4 text-right text-sm text-slate-600 tabular-nums">{q.sets}</td>
                <td className="px-5 py-4">
                  {q.active
                    ? <Badge color="green" dot="green">active</Badge>
                ...
                <td className="px-5 py-4 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button title="Edit" onClick={() => setEditing(q)}
                      className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded">
                      <Edit className="w-4 h-4" />
                    </button>
                    <button title="Delete"
      ...
      {(creating || editing) && (
        <QuestionEditor onClose={() => { setCreating(false); setEditing(null); }} editing={editing} />
      )}
    </div>
  );
}

function QuestionEditor({ onClose, editing }) {
  const [options, setOptions] = useState(editing
    ? [
        { letter: 'A', text: 'Drop all rows with missing values and proceed to modeling.', correct: false },
        { letter: 'B', text: 'Document the gap, assess impact on the business goal, and decide on a treatment strategy in Phase 3.', correct: true },
        { letter: 'C', text: 'Impute the missing values with the column mean immediately.', correct: false },
        { letter: 'D', text: 'Re-collect the data from the source system before continuing.', correct: false },
      ]
    : [
  ...
  return (
    <div onClick={onClose}
      className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-6 overflow-y-auto">
      <div onClick={(e) => e.stopPropagation()}
        className="bg-white rounded-xl shadow-2xl w-full max-w-3xl my-8 p-6 max-h-[90vh] overflow-y-auto">
        <h2 className="text-lg font-bold text-slate-900 mb-4">
        ...
        <div className="space-y-4">
          <Field label="Question stem">
            <textarea rows={3} defaultValue={editing?.stem ?? ''}
              placeholder="In CPMAI Phase 2, …"
              className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg font-medium" />
          </Field>

          <div className="grid grid-cols-2 gap-3">
            <Field label="CPMAI phase">
              <select defaultValue={editing?.phase ?? ''}
                className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg">
                <option value="">— select —</option>
                <option value="BU">BU — Business Understanding</option>
                <option value="DU">DU — Data Understanding</option>
                <option value="DP">DP — Data Preparation</option>
                <option value="MD">MD — Modeling</option>
                <option value="EV">EV — Model Evaluation</option>
                <option value="DE">DE — Model Operationalization</option>
              </select>
            </Field>
            <Field label="Difficulty">
              <select defaultValue={editing?.difficulty ?? 'medium'}
            ...
            <Field label="Domain (optional)">
              <input defaultValue={editing?.domain ?? ''} placeholder="Data Understanding > Quality"
                className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg" />
            </Field>
            <Field label="Task (optional)">
              <input placeholder="Identify and document gaps"
                className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg" />
            </Field>
          </div>

          <Field label="Enablers (comma-separated)">
            <input placeholder="Data profiling, Quality metrics"
              className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg" />
          </Field>

          <Field label="General explanation (shown after submit)">
            <textarea rows={2}
              className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg" />
          </Field>
          ...
          <div className="border-t border-slate-200 pt-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-slate-500">Mark exactly one as correct. Reasoning is shown after the candidate submits.</p>
              </div>
              <button className="text-xs px-3 py-1.5 bg-slate-100 hover:bg-slate-200 rounded">+ Add option</button>
            </div>
            {correctCount !== 1 && (
              <div className="bg-amber-50 border border-amber-200 text-amber-800 text-xs p-2 rounded mb-3 flex items-center gap-2">
                <AlertCircle className="w-3.5 h-3.5" />
            ...
            <div className="space-y-3">
              {options.map((opt, i) => (
                <div key={i} className={"border rounded-lg p-3 " + (opt.correct ? 'border-emerald-300 bg-emerald-50/40' : 'border-slate-200 bg-white')}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="w-7 h-7 rounded-full border-2 border-slate-300 flex items-center justify-center font-bold text-xs">{opt.letter}</span>
                    <label className="flex items-center gap-1 text-xs text-slate-700">
                      <input type="radio" name="correct" checked={opt.correct} onChange={() => setCorrect(i)} />Correct
                    </label>
                    <div className="flex-1" />
                    <button className="text-xs text-rose-600 hover:underline">Remove</button>
                  </div>
                  <input defaultValue={opt.text} placeholder="Option text"
                    className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg mb-2" />
                  <textarea rows={2}
                    placeholder={opt.correct ? 'Why this option is correct…' : 'Why this option is wrong…'}
                    className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg" />
                </div>
              ))}
            </div>
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-200">
          <label className="flex items-center gap-2 text-sm text-slate-700">
            <input type="checkbox" defaultChecked={editing?.active ?? true} />Active (visible to learners)
          </label>
          <div className="flex gap-2">
            <button onClick={onClose}
              className="px-4 py-2 text-sm text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50">Cancel</button>
