 ...
 */
/**
 * Interactive layer for /exams. The SERVER page fetches the set list +
 * banner copy anonymously so every exam card ships in the crawlable
 * initial HTML. On mount we refetch the sets once WHEN SIGNED IN so
 * per-user attempt counts appear; anonymous visitors keep the
 * server-rendered list untouched.
 */
export function ExamsListClient({ initialSets, initialAnonBanner }: {
  initialSets: ExamSetSummaryOut[] | null;
  initialAnonBanner: string | null;
}) {
  const [sets, setSets] = useState<ExamSetSummaryOut[] | null>(initialSets);
  const [me, setMe] = useState<UserOut | null | undefined>(undefined);
  const [error, setError] = useState<string | null>(null);
  const [anonBanner, setAnonBanner] = useState(
    initialAnonBanner?.trim() || DEFAULT_ANON_BANNER);

  useEffect(() => {
    auth.me()
      .then((u) => {
        setMe(u);
        // Signed-in: refetch so attempt counts (user-specific) load.
        examsApi.listSets().then(setSets)
          .catch((e) => { if (!initialSets) setError(errMsg(e)); });
      })
      .catch(() => {
        setMe(null);
        // Anonymous with no server data (API was down at render time):
        // one client-side retry so the page still works.
        if (!initialSets) {
          examsApi.listSets().then(setSets)
            .catch((e) => setError(errMsg(e)));
        }
      });
    if (!initialAnonBanner) {
      content.landing()
        .then((c) => {
